...
const Photo: React.FC<PropsPhoto> =
  ({ image, photoPostPage, handleDeletePhoto, showPhotos, handleChangeOrderPhoto, variants }) => {
  ...
  return (
    <motion.li className={styleContainerPhoto} variants={variants}>
      ...
      <img className={Styles.photo__image} src={image.image} alt={`photography ${image.type}`} title={`photography ${image.type}`}/>
